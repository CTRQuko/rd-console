...
from ..models.join_token import JoinToken, generate_join_token
...
def _to_out(t: JoinToken, now: datetime) -> JoinTokenOut:
    return JoinTokenOut(
        ...
        token_prefix=t.token_prefix,  # campo persistido (cierra VULN-04)
        label=t.label,
...
def _to_create_out(t: JoinToken, plaintext: str, now: datetime) -> JoinTokenCreateOut:
    ...
    El plaintext viene por parámetro porque tras VULN-04 el modelo NO
    persiste el plaintext — solo el `token_hash`. Lo recibe el handler
    al crear y lo entrega aquí una sola vez.
    """
...
def create_join_token(
    ...
    # Tras VULN-04: generamos plaintext + hash + prefix; persistimos
    # solo hash + prefix. El plaintext sale UNA VEZ en la respuesta.
    ...
    return _to_create_out(row, plaintext, now)
...
def revoke_or_delete_join_token(
    ...
    * `?hard=true` → delete: removes the row. Audit `JOIN_TOKEN_DELETED`
    ...
    if hard:
        ...
        payload = f"id={row.id} prefix={row.token_prefix} label={(row.label or '')[:64]}"
        session.delete(row)
