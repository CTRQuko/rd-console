...
const _DE = React;
const { useState: _deS, useEffect: _deE, useRef: _deR, useMemo: _deM, useCallback: _deCB } = React;
...
function useDashboardLayout() {
  const [layout, setLayout] = _deS(loadLayout);
  const [edit, setEdit] = _deS(false);
  const [draft, setDraft] = _deS(null); // copia mientras edita
...
function DashboardGrid({ layout, edit, onChange, children }) {
  const gridRef = _deR(null);
  const [drag, setDrag] = _deS(null); // { id, mode: 'move'|'resize', startX, startY, original }

  const childrenById = _deM(() => {
    const map = {};
  ...
  const [containerWidth, setContainerWidth] = _deS(0);
  _deE(() => {
  ...
  const [dragCursor, setDragCursor] = _deS(null);
  _deE(() => {
...
// Exponer al global para que Dashboard.jsx los consuma
