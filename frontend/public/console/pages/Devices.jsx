...
function TagInput({ value, onChange, catalog, onCreateTag }) {
  const [draft, setDraft] = _dvS("");
  const [open, setOpen] = _dvS(false);
  const ref = _dvR(null);
...
function EditableRow({ label, value, onChange, mono, render }) {
  const [editing, setEditing] = _dvS(false);
  const inputRef = _dvR(null);
  _dvE(() => { if (editing) inputRef.current?.focus(); }, [editing]);
  return (
    <>
...
function _DvKebab({ items, onClick }) {
  const [open, setOpen] = _dvS(false);
  const [openUp, setOpenUp] = _dvS(false);
  const ref = _dvR(null);
  _dvE(() => {
    if (!open) return;
    const onDoc = (e) => { if (!ref.current?.contains(e.target)) setOpen(false); };
