// @ts-nocheck
// Mechanically ported from public/console/pages/Settings.jsx
...
function ServidorPanel() {
  const [dbStatus, setDbStatus] = useState("idle"); // idle | checking | ok | error
...
function CreateTokenModal({ open, onClose, onSubmit }) {
  ...
  const [form, setForm] = useState({ name: "", user: operators[0], expiry: "365" });
...
function NewTokenSecretModal({ open, token, onClose }) {
  ...
  useEffect(() => { if (open) setCopied(false); }, [open, token?.secret]);
...
function AuthPanel({ embedded = false } = {}) {
  const [providers, setProviders] = useState({ ldap: true, oidc: false, saml: false, mfa: true });
  const rows = (
...
// ─── Settings shell ───────────────────────────────────────
export function SettingsPage({ route, navigate, theme, setTheme }) {
    ...
    <div className="cm-page">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "220px 1fr", gap: 24, alignItems: "flex-start" }}>
        ...
        <div>
          ...
          {sub === "usuarios"  && <UsersPage embedded={true} />}
